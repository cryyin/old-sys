...
from oldcare.openpose.tf_pose.common import to_str
from oldcare.openpose.tf_pose import common
...
class BaseNetwork(object):
    ...
    def separable_conv(self, input, k_h, k_w, c_o, stride, name, relu=True, set_bias=True):
        with tf.arg_scope([tf.batch_norm], decay=0.999, fused=common.batchnorm_fused, is_training=self.trainable):
            output = tf.separable_convolution2d(input,
                                                  num_outputs=None,
                                                  stride=stride,
            ...
            output = tf.convolution2d(output,
                                        c_o,
                                        stride=1,
                                        kernel_size=[1, 1],
                                        activation_fn=common.activation_fn if relu else None,
    ...
    def convb(self, input, k_h, k_w, c_o, stride, name, relu=True, set_bias=True, set_tanh=False):
        with tf.arg_scope([tf.batch_norm], decay=0.999, fused=common.batchnorm_fused, is_training=self.trainable):
            output = tf.convolution2d(input, c_o, kernel_size=[k_h, k_w],
                                        stride=stride,
                                        normalizer_fn=tf.batch_norm,
    ...
    def se_block(self, input_feature, name, ratio=8):
        ...
        ref : https://github.com/kobiso/SENet-tensorflow-tf/blob/master/nets/attention_module.py
        """
